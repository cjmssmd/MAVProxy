...
class ConsoleFrame(wx.Frame):
    ...
    def __init__(self, state, title):
        ...
        self.control = wx.TextCtrl(self.panel, size = (800,150), style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_AUTO_URL)
    ...
    def on_button(self, event):
        '''handle button click, event is a wx.Event'''
        '''pass the matching MPButton object into the pipe'''
        clickedbtn=event.GetEventObject()
        for btn in self.buttons:
            if btn.label == clickedbtn.GetLabelText():
                self.state.child_pipe_send.send(btn)
