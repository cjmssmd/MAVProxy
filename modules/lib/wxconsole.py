...
class MessageConsole(textconsole.SimpleConsole):
    ...
    def child_task(self):
        ...
        app.frame.Show()
        #app.frame.ShowFullScreen(show=True)
        app.MainLoop()
    ...
    def watch_thread(self):
        ...
        try:
            while True:
                ...
                elif isinstance(msg, MPMenuItem):
                    if self.menu_callback is not None:
                        self.menu_callback(msg)
                elif isinstance(msg, MPButton):
    ...
    def set_button(self, label, command, callback):
        if self.is_alive():
            self.parent_pipe_send.send(MPButton(label, command))
